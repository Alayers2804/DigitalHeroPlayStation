...
class BookingController extends Controller
{
    ...
    public function midtransCallback(Request $request)
    {
        ...
        // Debugging Transaction ID Matching
        Log::info('🔍 Checking transaction_id:', [
            'Received order_id' => $request->order_id,
            'Database transaction_ids' => Booking::pluck('transaction_id')
        ]);
        ...
        if (!$booking) {
            Log::error('❌ Midtrans Callback Error: Booking Not Found', [
                'Received order_id' => $request->order_id,
                'Available transaction_ids' => Booking::pluck('transaction_id')
            ]);
        ...
        Log::info('✅ Booking Found!', [
            ...
            'Received Status' => $request->transaction_status
        ]);
        ...
        switch ($request->transaction_status) {
            ...
            default:
                Log::warning('⚠️ Unhandled Transaction Status', [
        ...
        if (!$booking->save()) {
            Log::error('❌ Failed to Update Booking Status', [
                'order_id' => $booking->transaction_id,
                'Attempted Status' => $booking->status
            ]);
